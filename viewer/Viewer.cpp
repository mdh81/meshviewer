...
void Viewer::RenderLoop::draw() {
    ...
    do {
        ...
        if (!once)
            std::cerr << "Is Window Resized =  " << viewer->windowResized << std::endl;
        ...
        if (!once)
            std::cerr << "After Window Resized" << std::endl;
        ...
        if (!once)
            std::cerr << "Calling scene::render()" << std::endl;
        ...
        if (!once) {
            once = true;
            std::cerr << "Stopping this mother fucker" << std::endl;
            //exit(-1);
        }
