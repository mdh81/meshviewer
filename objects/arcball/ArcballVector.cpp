...
namespace mv::objects {
    ...
    ArcballVector::ArcballVector(common::DisplayDimensions const& displayDimensions)
    ...
    void ArcballVector::generateRenderData() {
        if (readyToRender) {
            return;
        }
        ...
        size_t dataSize;
        {
            ...
            unsigned index = 0;
            std::array<Point2D, 7> textureCoords = { Point2D {0, 0},
                                                     Point2D {1, 0},
                                                     Point2D {1, 1},
                                                     Point2D {0, 1},
                                                     Point2D {1, 0.5},
        ...
        numConnectivityEntries = static_cast<unsigned>(tris.size() * 3);

        generateColors();
    }

    void ArcballVector::generateColors() {
        if (readyToRender) return;

    }

    void ArcballVector::fadeOut() {

    }

    void ArcballVector::setTransforms() {
    ...
    void ArcballVector::constructGeometry(common::Points &vertices, math3d::types::Tris& tris, float lineWidthCamera) {

        // Create a quad in camera coordinates. The dimensions are shaft width x line width
        auto shaftWidth = 0.8f;
        vertices.push_back({0, 0, 0});
        vertices.push_back({shaftWidth, 0, 0});
        vertices.push_back({shaftWidth, lineWidthCamera, 0});
        vertices.push_back({0, lineWidthCamera, 0});

        tris.push_back({0, 1, 2});
        tris.push_back({2, 3, 0});

        // Create a tri in camera coordinates for the vector's tip. The dimensions are head width * line width
        auto headWidth = 0.2f;
        vertices.push_back({1, lineWidthCamera/2, 0 });
        vertices.push_back({1-headWidth, +lineWidthCamera * 1.5f, 0});
        vertices.push_back({1-headWidth, -lineWidthCamera/2, 0});

        tris.push_back({4, 5, 6});
    }
